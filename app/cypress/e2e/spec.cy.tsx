...
describe("Testing the UI with the reponse from GET /lightning", () => {
  it("Opens the application and waits for API response, then checks each component on the console to ensure values match", () => {
    ...
    cy.get("[aria-label=loading-text]").should(
      "contain.text",
      "Loading, please wait..."
    );
    let res;
    cy.wait("@getLightning").then((interception) => {
      ...
      highestStrikes(res.features).forEach((feature, index) => {
        cy.get(`[aria-label=highest-strikes-table-cell-${index}-name]`).should(
          "contain.text",
          feature.name
        );
        cy.get(
          `[aria-label=highest-strikes-table-cell-${index}-20km-strikes]`
        ).should("contain.text", feature.twentyKmStrikes);
        cy.get(
          `[aria-label=highest-strikes-table-cell-${index}-100km-strikes]`
        ).should("contain.text", feature.hundredKmStrikes);
        cy.get(
          `[aria-label=highest-strikes-table-cell-${index}-total-strikes]`
        ).should("contain.text", feature.totalStrikes);
        cy.get(
          `[aria-label=highest-strikes-table-cell-show-coordinates-${index}]`
        ).click();

        cy.get(`[aria-label=strike-dialog-${index}-name]`).should(
          "contain.text",
          feature.name
        );
      ...
      alertStrikes(res.features).forEach((feature, index) => {
        cy.get(`[aria-label=alert-strikes-table-cell-${index}-name]`).should(
          "contain.text",
          feature.name
        );
        cy.get(`[aria-label=alert-strikes-table-cell-${index}-area]`).should(
          "contain.text",
          feature.area
        );
        cy.get(
        ...
        cy.get(
          `[aria-label=alert-strikes-table-cell-${index}-total-strikes]`
        ).should("contain.text", feature.totalStrikes);
        cy.get(
          `[aria-label=alert-strikes-table-cell-show-coordinates-${index}]`
        ).click();
      ...
      res.features.forEach((feature, index) => {
        ...
        cy.get(`[aria-label=full-table-cell-${index}-name]`).should(
          "contain.text",
          feature.properties.name
        );
        cy.get(`[aria-label=full-table-cell-${index}-area]`).should(
          "contain.text",
          feature.properties.area
        );
        cy.get(`[aria-label=full-table-cell-${index}-volcano-type]`).should(
          "contain.text",
          feature.properties.volcanoType
        );
        cy.get(`[aria-label=full-table-cell-${index}-20km-strikes]`).should(
          "contain.text",
          feature.properties.twentyKmStrikes
        );
        cy.get(`[aria-label=full-table-cell-${index}-100km-strikes]`).should(
          "contain.text",
        ...
        cy.get(`[aria-label=strike-dialog-${index}-area]`).should(
          "contain.text",
          feature.properties.area
        );
        cy.get(`[aria-label=strike-dialog-bar-icon]`).click();
