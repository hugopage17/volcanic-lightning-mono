  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center" }}>
          <Tooltip
            arrow
            sx={{ marginRight: "4px" }}
            title={`${strike?.properties?.name} at ${
              strike?.properties?.severity === "error" ? "high" : "medium"
            } alert for lightning strikes`}
          >
            <WarningIcon color={strike?.properties?.severity as any} />
          </Tooltip>
        ...
        <Tabs
          ...
          aria-label="strike dialog tabs"
        >
          <Tab
            icon={<MapIcon />}
            aria-label="strike-dialog-map-icon"
            {...a11yProps(0)}
          />
          <Tab
            icon={<InsertChartIcon />}
            aria-label="strike-dialog-bar-icon"
            {...a11yProps(1)}
          />
        </Tabs>
      </DialogTitle>
      ...
      <DialogContent sx={{ overflow: "hidden" }}>
        {tabValue === 0 && (
          <MapComponent loadedMap={loadedMap} strike={strike} />
        )}
        {tabValue === 1 && <BarGraphComponent strike={strike} />}
      </DialogContent>
    </Dialog>
  );
};
