...
const Map = ({ features, center }: Props): JSX.Element => {
  ...
  const [regions, toggleRegion] = React.useState<string[]>(
    getRegions(features)
  );

  React.useEffect(() => toggleRegion(getRegions(features)), [features]);
  ...
  const isActiveRegion = (region: string) =>
    !!regions.find((r) => r === region);
